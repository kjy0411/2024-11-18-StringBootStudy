...
public class BusanFoodRestController {
	@Autowired
	public BusanFoodService bService;
	...
	public Map main_data() {
		...
		List<BusanInfoEntity> iList=bService.busanInfoData();
		map.put("mp", vo);
		map.put("mList", mList);
		map.put("lList", lList);
		map.put("iList", iList);
	...
	public BusanFoodEntity food_detail(@RequestParam("fno") int fno) {
		BusanFoodEntity vo=bService.foodDetailData(fno);
		return vo;
	}
	...
	public BusanInfoEntity info_detail(@RequestParam("no") int no) {
		BusanInfoEntity vo=null;
		return vo;
	}
